...
/** Downscales an image to a REF_WIDTH JPEG in the cache (once) and returns it as a data URI. */
async function toDataUri(source: string, cacheName: string) {
  mkdirSync(`${CACHE_DIR}/fal-refs`, { recursive: true });
  const jpeg = `${CACHE_DIR}/fal-refs/${cacheName}`;
  if (!existsSync(jpeg)) {
    const proc = Bun.spawn(["ffmpeg", "-v", "error", "-y", "-i", source, "-vf", `scale='min(${REF_WIDTH},iw)':-2`, "-q:v", "3", jpeg]);
...
/** A generated frame (the previous clip's last frame) as an inline reference. New every step, so not cached. */
export function uploadFile(path: string) {
  return toDataUri(path, `frame-${crypto.randomUUID()}.jpg`);
}
...
export async function generateVideo({ durationSecs, ...req }: VideoRequest) {
  ...
  return traced(
    ...
    logged,
    async () => {
      // Deliberately not retried: a submit that reached fal is a paid generation, and a retry could pay twice.
      ...
      // Paid for from here: network blips must not abandon it.
      ...
      /** Downloads the clip, stores it in R2 and returns the stable URL. Safe to call more than once. */
      ...
      logEvent({ kind: "video", label: "fal clip ready", response: { requestId, seconds: Math.round((Date.now() - started) / 1000), inference: result.timings?.inference } });
      return { taskId: id, url: remoteUrl, remote: true as const, finalize, creditCost: costUsd };
    },
