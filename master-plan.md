...
```
[Start screen: HyperFrames "PRISON ESCAPE" title] → click Start (unmutes, starts music)
      ↓
...
- `common-generated-assets/characters/*.txt` and `common-generated-assets/environments/*.txt`: image-generation prompts (awaiting validation).
  Naming: `larry-protagonist`, `guard1`…`guard10`, `prisoner1`…`prisoner10`, and environment ids.
- `asset-personalities/characters/*.md` and `asset-personalities/environments/*.md`: gameplay notes the LLMs will reference.
  Characters list personality, wants, weakness, locations, how they help, how Larry can win them over, danger and voice.
  Environments list security, steps to the nearest exit, guards and inmates present, items, hazards, ways in and out,
  high-innovation ideas, and what gets Larry caught.
...
- [ ] **Intro master video**: prompts in `common-generated-assets/videos/intro-keyframe.txt` and `intro.txt`, generated by `scripts/gen-intro.ts`.
  - Step 1: Nano Banana Pro keyframe of Larry in his cell, built from his sheet and the cell image.
  - Step 2: MiniMax-H3 image-to-video, 15s, 480p, `EXPRESS`, with the keyframe as both first and last frame, so it loops seamlessly.
  - It has 7 fast cuts: Larry close-up → top-down aerial of the prison → guard tower → sergeant in the hall → inmates on the tier → spoon on the vent → back to Larry.
  - No typography in the video: "PRISON ESCAPE" is the `intro-title` HyperFrames overlay, and the prompt bar reads "Larry's in his cell. What should Larry do?".
...
- [x] `title`: "PRISON ESCAPE" spy HUD, red/blue police sweep (first draft)
